...
public class FiltersAndMapExercise {
    ...
    public static List<String> findPersonByName(String name) {
        ...
        personName = input.filter((string) -> string.equals(name)).collect(toList());
    ...
    public static List<Person> findPersonByAgeAndName(List<Person> personList, String name,
                                                      Integer age) {
        List<Person> persons = new ArrayList<>();
        //TODO #4 find person by given name and age

        persons = personList.stream().filter(person -> Objects.equals(person.getName(), name) && Objects.equals
                (person.getAge(), age)).collect(toList());
    ...
    public static int product() {
        ...
        product = input.reduce(1, (integer, integer2) -> integer * integer2);
    ...
    public static int findTheYoungestPerson(List<Person> personList) {
        ...
        min = personList.stream().map(Person::getAge).reduce(+999, Integer::min);
    ...
    public static List<Nationality> findNationalityByName(List<Person> personList) {
        ...
        //TODO #14
        // HINT use startsWith() for finding the names that starts with J, and distinct() to
        // remove duplicates
    ...
    public static Optional<Integer> findSquareDivisibleBy3() {
        ...
        firstSquareDivisibleByThree = someNumbers.
                stream().
                filter(integer -> Math.sqrt(integer) == ((int) Math.sqrt(integer))).
    ...
    public static int sumStringLength() {
        ...
        sum = strings.stream().map(String::length).reduce(0, Integer::sum);
    ...
    public static int sumSalary(List<Person> personList) {
        ...
        sum = personList.stream().map(Person::getSalary).reduce(0, Integer::sum);
