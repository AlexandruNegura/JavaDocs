...
public class PartitioningAndGroupingTest {
    ...
    public void partitionDishes() {
        ...
        partitionedDishes = Dish.menu.stream().collect(partitioningBy((Dish dish) -> dish.getCalories() > 380));
    ...
    public void groupDishes() {
        ...
        groupedDishes = Dish.menu.stream().collect(groupingBy(dish -> {
                    if (dish.getCalories() <= 400) {
                        ...
                        if (dish.getCalories() <= 700 && dish.getCalories() > 400) {
...
//
    ...
    public void groupCounting() {
        ...
        groupedDishes = Dish.menu.stream().filter(dish -> dish.getCalories() >= 140).collect(groupingBy
